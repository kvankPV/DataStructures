...
namespace ds::utils {
	...
	template <class Container>
	class MyAnalyzerPop final : public MyAnalyzer<Container>
	{
	public:
		explicit MyAnalyzerPop(const std::string& name);
	...
	//--
	template <class Container>
	MyAnalyzerPop<Container>::MyAnalyzerPop(const std::string& name) :
	MyAnalyzer<Container>(name)
	{
	}

	template <class Container>
	void MyAnalyzerPop<Container>::executeOperation(Container& container)
	{
		container.pop();
	}
	...
	inline MyAnalyzers::MyAnalyzers() : CompositeAnalyzer("IDK")
	{
		...
		this->addAnalyzer(std::make_unique<MyAnalyzerPop<adt::TwoLists<int, int>>>("twoLists-pop"));
	}
}
