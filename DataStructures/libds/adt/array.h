...
namespace ds::adt {
    ...
    {
	    if (const Array<T>& otherArray = dynamic_cast<const Array<T>&>(other); base_ != otherArray.base_ || this->size() != otherArray.size())
        {
    ...
    T Array<T>::access(long long index) const
    {
        ...
        {
            throw structure_error("Nespravny index!");
        }
    ...
    T CompactMatrix<T>::access(long long index1, long long index2) const
    {
        ...
        {
            throw structure_error("Wrong index! ");
        }
        size_t mappedIndices = mapIndices(index1, index2);
        return this->getSequence()->access(mappedIndices)->data_;
    }
    ...
    void CompactMatrix<T>::set(T element, long long index1, long long index2)
    {
        ...
        size_t mappedIndices = mapIndices(index1, index2);
        this->getSequence()->access(mappedIndices)->data_ = element;
    }
    ...
    {
        return (index1 - dimension1_.getBase()) * dimension2_.getSize() +
            (index2 - dimension2_.getBase());
    }
