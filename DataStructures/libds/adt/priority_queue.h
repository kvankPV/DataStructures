...
namespace ds::adt {
    ...
    void TwoLists<P, T>::push(P priority, T data)
    {
        PQItem<P, T>* queueData;
        if (
            longSequence_->size() == 0 ||
            !shortSequence_->isEmpty() &&
            shortSequence_->accessFirst()->data_.priority_ > priority
            ) {
            if (shortSequence_->size() == shortSequence_->getCapacity() && shortSequence_->accessFirst()->data_.priority_ > priority) {

                ShortSequenceBlockType* shortSequenceBlock = shortSequence_->accessFirst();
                LongSequenceBlockType* longSequenceBlock = &longSequence_->insertLast();

                longSequenceBlock->data_.priority_ = shortSequenceBlock->data_.priority_;
                longSequenceBlock->data_.data_ = shortSequenceBlock->data_.data_;
                shortSequence_->removeFirst();

            }
            if (
                shortSequence_->isEmpty() ||
                priority < shortSequence_->accessLast()->data_.priority_
                ) {
                ...
                if (priority > shortSequence_->accessFirst()->data_.priority_) {
                    ...
                    ShortSequenceBlockType* blockWithProperty = shortSequence_->findBlockWithProperty(
                        [&priority, this](ShortSequenceBlockType* block) {
                            return block->data_.priority_ <= priority;
                        }
                    );
                    queueData = &shortSequence_->insertBefore(*blockWithProperty).data_;
                }
            }
        }
        else {
    ...
    T& TwoLists<P, T>::peek()
    {
        ...
        {
            throw structure_error("Priority queue is empty!");
        }
    ...
    T TwoLists<P, T>::pop()
    {
        ...
        {
            amt::SinglyLS<PQItem<P, T>>* previousLongSeq = this->longSequence_;
            this->longSequence_ = new amt::SinglyLS<PQItem<P, T>>();
            this->shortSequence_->changeCapacity(
                static_cast<size_t>(ceil(sqrt(previousLongSeq->size()))));
            while(!previousLongSeq->isEmpty())
            {
                LongSequenceBlockType* blok = previousLongSeq->accessFirst();
                this->push(blok->data_.priority_,blok->data_.data_);
                previousLongSeq->removeFirst();
            }
            delete previousLongSeq;
            previousLongSeq = nullptr;
        }
        return result;
    }
