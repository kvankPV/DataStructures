...
namespace ds::amt {
	...
	template<typename DataType, size_t K>
    size_t ImplicitHierarchy<DataType, K>::level(size_t index)
    {
	...
	template<typename DataType, size_t K>
	[[nodiscard]] size_t ImplicitHierarchy<DataType, K>::degree(const size_t index) const
	{
		const size_t current_level = this->level(index);
		const size_t index_last = this->size() - 1;
		const size_t depth = this->level(index_last);
		...
		{
			const size_t last_parent_index = this->indexOfParent(index_last);

			if (index < last_parent_index)
			{
				return K;
			}
	...
	{
		if (size_t index = this->indexOfParent(node); INVALID_INDEX != index)
		{
	...
	{
		if (const size_t size = this->size(); size != 0)
		{
