...
namespace ds::amt {
	...
	public:
		...
		BlockType& emplaceSon(BlockType& parent, size_t son_order) override;
	...
	{
		std::function<void(BlockType*)> clear_node;
		clear_node = [&](BlockType* node)
			{
				this->memoryManager_->releaseMemory(node);
			};
		this->processPostOrder(this->root_, clear_node);
		root_ = nullptr;
    ...
    size_t ExplicitHierarchy<BlockType>::size() const
	{
		...
		{
			return this->nodeCount(*this->root_);
		}
		return 0;
	}
    ...
    bool ExplicitHierarchy<BlockType>::equals(const AMT& other)
	{
		...
		compare = [&](BlockType* myBlock, BlockType* otherBlock) -> bool
		{
			...
			if (myBlock == nullptr || otherBlock == nullptr)
				return false;
			if (this->degree(*myBlock) != otherHierarchy->degree(*otherBlock))
			{
				return false;
			}
			if (!(myBlock->data_ == otherBlock->data_))
			{
				return false;
			}
	...
	{
		this->root_ = this->memoryManager_->allocateMemory();
		return *this->root_;
	}
	...
	{
		MemoryBlock<BlockType*>* nodeSons = node.sons_->access(sonOrder);
		if (nodeSons != nullptr)
		{
			return nodeSons->data_;
		}
		return nullptr;
	}

	template<typename DataType>
    auto MultiWayExplicitHierarchy<DataType>::emplaceSon(BlockType& parent, size_t son_order) -> BlockType&
	{
		BlockType* new_son = this->memoryManager_->allocateMemory();
		parent.sons_->insert(son_order).data_ = new_son;
		new_son->parent_ = &parent;
		return *new_son;
	}
	...
	{
		MemoryBlock<BlockType*>* sons_block = parent.sons_->access(sonOrder);
		BlockType* previous_son = sons_block->data_;
		sons_block->data_ = newSon;
		if (previous_son != nullptr)
		{
			previous_son->parent_ = nullptr;
		}
		if (newSon != nullptr)
		{
	...
	{
		MemoryBlock<BlockType*>* sons_block = parent.sons_->access(sonOrder);
		BlockType* deleted_son = sons_block->data_;
		//std::function<void(BlockType*)> b;
		this->processPostOrder(deleted_son, [&](BlockType* b)
			{
	...
	{
		size_t result = 0;
		for(auto son = node.sons_->begin(); son != node.sons_->end(); ++son)
		{
			if (*son != nullptr)
			{
				++result;
	...
	{
		MemoryBlock<BlockType*>* sons_block = node.sons_->access(sonOrder);
		if (sons_block != nullptr)
		{
			return sons_block->data_;
		}
		return nullptr;
	}
	...
	{
		BlockType* new_son = this->memoryManager_->allocateMemory();
		parent.sons_->access(sonOrder)->data_ = new_son;
		new_son->parent_ = &parent;
		return *new_son;
	}
